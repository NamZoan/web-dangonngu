...
@section('content')
    <main id="content" role="main" class="main">
        {!! html()->form('POST', route('admin_product_update', $product->id))->class('validasi')->attributes(['novalidate'])->open() !!}
        <!-- Content -->
                        ...
                        <div class="row">
                            <div class="col-lg-7">
                                <div class="card mb-3 mb-lg-5">
                                    <div class="card-header">
                                        ...
                                        <div class="col-md-10">
                                            {!! html()->input('hidden', 'image', $product->image)->class('form-control')->id('file-name')->placeholder('Đường dẫn link ảnh')->aria('label', 'Đường dẫn link ảnh') !!}
                                        </div>
                                    </div>
                                    <div class="card-body">
                                        <div id="attachFilesNewProjectLabel"
                                            class="dropzone-custom custom-file-boxed dz-clickable">
                                            {!! html()->img($product->image ? asset($product->image) : asset('userfiles/noimage.jpg'))->class('')->id('file-url')->alt('Image Description') !!}
                                            ...
                                            <div class="upload__btn-box">
                                                <label class="upload__btn">
                                                    ...
                                                    <input type="text" multiple="" data-max_length="20"
                                                        value='{{ $product->multiple_image }}' name="multiple_image"
                                    ...
                                    {!! html()->input('text', 'link_affiliate', $product->link_affiliate)->class('form-control')->id('link_affiliate')->placeholder('Đường dẫn')->aria('label', 'link_affiliate') !!}
                                    ...
                                    {!! html()->div()->class('input-group')->open() !!}
                                    {!! html()->input('text', 'model', $product->model)->class('form-control')->id('model')->placeholder('Thương hiệu')->aria('label', 'Tên thương hiệu') !!}
                                    {!! html()->div()->close() !!}
                                    {!! html()->div()->close() !!}
                                    {!! html()->div()->class('form-group col-6 col-sm-6')->open() !!}
                            ...
                            <div class="card col-lg-5">
                                <div class="card-body">
                                    ...
                                    {!! html()->div()->class('input-group')->open() !!}
                                    {!! html()->input('text', 'code', $product->code)->class('form-control')->id('code')->placeholder('Mã sản phẩm')->aria('label', 'code')->attributes(['required', 'onchange' => 'checkCode']) !!}
                                    ...
                                    {!! html()->div()->class('input-group')->open() !!}
                                    {!! html()->input('text', 'slug', $product->slug)->class('form-control')->id('slug')->placeholder('slug')->aria('label', 'slug')->attributes(['required', 'onchange' => 'updateSlug(' . $product->id . ',null,this.value,"products")']) !!}
                                    {!! html()->div()->class('invalid-tooltip')->open() !!}
                                    Vui lòng nhập đường dẫn
                                    {!! html()->div()->close() !!}
                                    {!! html()->div()->class('input-group-append') !!}
                                    ...
                                    <label class="toggle-switch" for="status">
                                        ...
                                        <span class="col-4 col-sm-3">
                                            {!! html()->checkbox('status', $product->status == 1 ? 1 : 0)->class('toggle-switch-input')->id('status') !!}
                                ...
                                <div class="tab-pane fade show {!! app()->getLocale() == $val->abbr ? 'active' : '' !!}" id="lang_{{ $val->abbr }}"
                                    ...
                                    <div class="row">
                                        <div class="col-lg-7">
                                            <div class="card mb-3 mb-lg-5">
                                                ...
                                                <div class="card-body">
                                                    ...
                                                    {!! html()->div()->class('input-group')->open() !!}
                                                    {!! html()->input('text', 'name[' . $val->abbr . ']', $product->getTranslation('name', $val->abbr))->class('form-control')->id('name')->placeholder('Tên sản phẩm')->aria('label', 'Tên sản phẩm')->attributes(['required', 'onchange' => 'updateSlug(' . $product->id . ',this.value,null,"products")']) !!}
                                                    {!! html()->div()->class('invalid-tooltip')->open() !!}
                                                ...
                                                <div class="card-body">
                                                    ...
                                                    {!! html()->div()->class('input-group')->open() !!}
                                                    {!! html()->input('number', 'price[' . $val->abbr . ']', $product->getTranslation('price', $val->abbr))->class('form-control')->id('price')->placeholder('Giá sản phẩm')->aria('label', 'Tên danh mục')->attributes(['required']) !!}
                                                    {!! html()->div()->class('invalid-tooltip')->open() !!}
                                                    ...
                                                    <select class="form-control" id="unit"
                                                        ...
                                                        @foreach ($languages as $lang)
                                                            <option value="{{ $lang->unit }}" {!! $lang->unit == $product->getTranslation('unit', $val->abbr) ? 'selected' : '' !!}>
                                                                {{ $lang->unit }}</option>
                                                        @endforeach
                                                    </select>
                                                    {!! html()->div()->close() !!}
                                                    ...
                                                    {!! html()->div()->class('input-group')->open() !!}
                                                    {!! html()->input('text', 'made[' . $val->abbr . ']', $product->getTranslation('made', $val->abbr))->class('form-control')->id('made')->placeholder('Xuất xứ')->aria('label', 'Xuất xứ') !!}
                                                    {!! html()->div()->close() !!}
                                                    {!! html()->div()->close() !!}
                                                </div>
                                        ...
                                        <div class="col-lg-5">
                                            <div class="card mb-3 mb-lg-5">
                                                ...
                                                <div class="card-body">
                                                    ...
                                                    {!! html()->div()->class('input-group')->open() !!}
                                                    {!! html()->input('text', 'meta_title[' . $val->abbr . ']', $product->getTranslation('meta_title', $val->abbr))->class('form-control')->id('meta_title')->placeholder('meta_title')->aria('label', 'meta_title') !!}
                                                    {!! html()->div()->class('input-group-append') !!}
                                                    {!! html()->div()->close() !!}
                                                    {!! html()->div()->close() !!}
                                                    ...
                                                    {!! html()->div()->class('input-group')->open() !!}
                                                    {!! html()->input('text', 'meta_keyword[' . $val->abbr . ']', $product->getTranslation('meta_keyword', $val->abbr))->class('form-control')->id('meta_keyword')->placeholder('meta_keyword')->aria('label', 'meta_keyword') !!}
                                                    {!! html()->div()->close() !!}
                                                    {!! html()->div()->close() !!}
                                                    ...
                                                    {!! html()->div()->class('input-group')->open() !!}
                                                    {!! html()->input('text', 'meta_description[' . $val->abbr . ']', $product->getTranslation('meta_description', $val->abbr))->class('form-control')->id('meta_description')->placeholder('meta_description')->aria('label', 'meta_description') !!}
                                                    {!! html()->div()->close() !!}
                                                    {!! html()->div()->close() !!}
                                                </div>
                                            </div>
                                        ...
                                        <div class="col-lg-12">
                                            ...
                                            {!! html()->textarea('summary[' . $val->abbr . ']', $product->getTranslation('summary', $val->abbr))->class('form-control')->id('summary[' . $val->abbr . ']')->rows(10) !!}
                                            {!! html()->div()->close() !!}
                                        </div>
                                        <div class="col-lg-12">
                                            <label class="input-label">Mô tả sản phẩm<span
                                                    ...
                                                    buộc)</span></label>
                                            {!! html()->div()->class('quill-custom')->open() !!}
                                            {!! html()->textarea('description[' . $val->abbr . ']', $product->getTranslation('description', $val->abbr))->class('form-control')->id('description[' . $val->abbr . ']')->rows(10) !!}
                                            {!! html()->div()->close() !!}
                                        </div>
