@php
    ...
    $route = substr($routeName, strpos($routeName, '.') + 1);
    $slug = request()->route()->parameters()['slug'] ?? null;
    foreach ($languages as $key => $value) {
        if ($value->abbr == app()->getLocale()) {
            $flagDefault = $value->flag;
            $unit = $value->unit;
            $langDefault = $value->abbr;
        } else {
            $langs[$key]['flag'] = $value->flag;
            $langs[$key]['abbr'] = $value->abbr;
            $langs[$key]['name'] = $value->getTranslation('name', $value->abbr);
            $langs[$key]['url'] = route($route, $slug, true, $value->abbr);
        }
    }
@endphp
<script>
    var formatter = new Intl.NumberFormat('{{ app()->getLocale() }}', {
        style: 'currency',
        currency: '{{ $unit }}'
    });
</script>
<div class="header-main grid-container" data-sticky-container>
    <div class="header title-bar" data-sticky data-margin-top="0" data-options="marginTop:0;" data-sticky
        data-sticky-on="small">
        <div class="title-bar-left">
            <a href="{{ route('web_home', null, true, $langDefault) }}">
                <span class="title-bar-title">
                    <svg id="logo-9" width="152" height="19" viewBox="0 0 152 19" fill="none"
        ...
        <div class="title-bar-right ">
            <ul class="lang_dropdown dropdown menu" data-dropdown-menu>
                <li>
                    <button class="button" type="button" data-toggle="example-dropdown-bottom-left"
                        style="padding:0;border:none;background:none">{!! $flagDefault !!}</button>

                </li>
                <li>
                    <button type="button" class="burger" id="burger" data-toggle="menu">
    ...
    <div class="dropdown-pane change_languages" data-position="bottom" data-alignment="left"
        id="example-dropdown-bottom-left" data-dropdown data-auto-focus="true" data-close-on-click="true">
        @foreach ($langs as $lang)
            <a href="{{ $lang['url'] }}" class="lang grid-x grid-padding-x">
                <div class="cell shrink">
                    {!! $lang['flag'] !!}
                </div>
                <div class="text-left grid-y grid-padding-y align-center">
                    <span>{{ $lang['name'] }}</span>
                </div>
            </a>
        @endforeach
    </div>
    <div class="dropdown-pane bottom header-menu text-left" data-allow-bottom-overlap="true" data-alignment="right"
        data-h-offset="-20" data-v-offset="15" id="menu" data-position="bottom" data-dropdown
        data-close-on-click="true">
        <ul style="margin:0">
            <li><a class="link-menu" href="{{ route('web_home') }}">
                    <p>{{ GetTranslation(trans('header.home')) }}</p>
                </a></li>
            <li><a class="link-menu" href="javascript:void(0)">
                    <p>Chưa Có</p>
                </a></li>
            <li><a class="link-menu" href="javascript:void(0)">
                    <p>Chưa Có</p>
            ...
            <li><a class="link-menu" href="javascript:void(0)">
                    <p>{{ GetTranslation(trans('header.blogs')) }}</p>
                </a></li>
            @foreach ($information_global as $item)
                @if ($item->view_header == 1)
                    <li><a class="link-menu" href="{{ route('web.information', $item->slug) }}">
...
@if (request()->route()->getName() != $langDefault . '.web_checkout' &&
        request()->route()->getName() != $langDefault . '.web_cart')
    <div class="js-cd-cart">
        <a href="javascript:;" class="cd-cart__trigger text-replace">
        ...
        <div class="cd-cart__content">
            <div class="cd-cart__layout">
                <div class="cd-cart__header">
                    <h2>{{ GetTranslation(trans('cart.name')) }}</h2>
                </div>
                ...
                <div class="cd-cart__footer">
                    <a href="{{ route('web_cart', null, true, $langDefault) }}" class="cd-cart__checkout">
                        <em>{{ GetTranslation(trans('cart.view')) }}
